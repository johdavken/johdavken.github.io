...
test("Slate never writes through a bridge - it only reads and subscribes", () => {
  for (const file of SLATE_FILES) {
    ...
    for (const match of source.matchAll(/(\w+)\.subscribe\s*\(/g)) {
      assert.ok(["bridge", "connection", "admin", "controller", "theme", "display", "displayController"].includes(match[1]), `${file} subscribes to ${match[1]}`);
    }
  }
});
...
test("exactly one Slate file requests connection actions, and none asks the admin bridge for anything", () => {
  for (const file of SLATE_FILES) {
    const source = codeOnly(read(file));
    ...
    assert.doesNotMatch(source, /PolynStationRecipesBridge|PolynStationWeightProfilesBridge/, `${file} reaches a bridge phase 1 does not consume`);
  }
});
