...
function mainMobileBlock(){
  ...
  const landmark = styles.indexOf("#splitsSaveRecipe, #splitsSaveNextRecipe, #splitsLoadRecipe, #splitsUpdateRecipe{");
...
test("each icon button gets its own glyph via ::before, sized independently of the parent's font-size:0", () => {
  ...
  // Forces system-ui explicitly - the inherited button font left "›"
  // unrendered/tofu on at least one real font stack.
  assert.match(block, /#splitsSaveNextRecipe::before\{ content: "\+›"; font-size: 12px; line-height: 1; letter-spacing: -\.06em; font-family: system-ui,sans-serif; \}/);
...
test("the title row wraps on phone instead of forcing three pieces (title, Save icons, Recipe Book's own search field) onto one line", () => {
  const block = mainMobileBlock();
  assert.match(block, /\.splitsSavedRecipesPanel \.workspaceConfigurationSectionTitle\{\s*\n\s*flex-wrap: wrap;/);
  assert.match(block, /\.splitsSavedRecipesPanel \.mobileSavedRecipesSearch\{\s*\n\s*flex: 1 1 100%;\s*\n\s*order: 1;/);
});
