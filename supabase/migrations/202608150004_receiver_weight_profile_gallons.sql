...
-- Smart Hoppers' volume-based (non-cylindrical) geometry: usable_gallons,
-- for lines whose receiver hoppers are rated in gallons rather than
-- circumference/height. Same optional-on-read treatment already given to
-- usable_heights_in/circumferences_in by 202608080001: a profile saved
-- before this mode existed simply won't have usable_gallons, and that's
-- fine - only *malformed* geometry (wrong length, non-numeric, negative)
-- is rejected. Not applied yet; this is server-side hardening to match the
-- client-side validation already added in workspace-configuration-payloads.js.
-- The existing function already accepted extra JSONB keys without
-- complaint, so the client-side feature works against the
...
-- usable_heights_in/circumferences_in.
...
begin
  ...
  for v_index in 0..v_expected_layers - 1 loop
    ...
    if p_configuration_type = 'receiver_weight_profile' then
      ...
      if v_layer ? 'usable_gallons' then
        if jsonb_typeof(v_layer->'usable_gallons') <> 'array'
           or jsonb_array_length(v_layer->'usable_gallons') <> 6 then
          raise exception using errcode = '22023', message = 'invalid_hopper_usable_gallons';
        end if;
        for v_hopper_index in 0..5 loop
          if jsonb_typeof(v_layer->'usable_gallons'->v_hopper_index) <> 'number'
             or (v_layer->'usable_gallons'->>v_hopper_index)::numeric < 0 then
            raise exception using errcode = '22023', message = 'invalid_hopper_usable_gallon';
          end if;
        end loop;
      end if;
    else
