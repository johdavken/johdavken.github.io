...
test("mobile tool panels use one accessible Back to Tools header with the active tool as a heading", () => {
  ...
  assert.match(header, /<h1 id="mobileToolHeaderLabel">Production Summary<\/h1>/);
  assert.match(styles, /body\[data-mobile-tools="panel"\] #toolsBlock > summary\{ display:none; \}/);
  assert.match(styles, /min-width:44px;/);
});

test("shortcut entry points open their corresponding tool panel so Back to Tools always returns to the selector", () => {
  const quickScan = app.slice(app.indexOf('$("quickScanDosingScreenBtn")'), app.indexOf('$("quickProductionSummaryBtn")'));
  assert.match(quickScan, /setWorkspacePanel\("toolsBlock", \{ reveal: true \}\)/);
  assert.match(quickScan, /selectToolPanel\("recipeScanTool"\)/);
  const quickSummaryStart = app.indexOf('$("quickProductionSummaryBtn")');
  const quickSummary = app.slice(quickSummaryStart, app.indexOf('$("appFooterMain")', quickSummaryStart));
  assert.match(quickSummary, /selectToolPanel\("productionSummaryTool"\)/);
});
