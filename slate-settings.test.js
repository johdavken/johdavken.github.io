...
test("the picker offers every registered theme as a radio, marks the current one, and drives the controller", () => {
  ...
  assert.deepEqual(tiles.map(tile => tile.getAttribute("role")), ["radio", "radio"]);
  assert.deepEqual(tiles.map(tile => tile.getAttribute("aria-checked")), ["true", "false"]);
  ...
  assert.deepEqual(tiles.map(tile => tile.getAttribute("aria-checked")), ["false", "true"]);
...
test("with no controller the tiles are inert and the section says so; the later-preferences stub is present", () => {
  ...
  const tiles = view.element.querySelectorAll("[data-theme-choice]");
  assert.deepEqual(tiles.map(tile => tile.getAttribute("aria-checked")), ["false", "false"]);
