(function(root, factory){
  ...
  const FIELDS = "id,line_number,display_name,aliases,layer_count,hopper_counts,hopper_manufacturer,layer_a_position,hopper_geometry,hopper_naming_mode,is_active,metadata,created_at,updated_at";
  let client = null;
    ...
    async function save(id, values){
      ...
      try{
        const response = await adminClient.rpc("admin_save_line_configuration", {
          ...
          p_hopper_manufacturer:candidate.hopperManufacturer
        });
        if (response.error) throw response.error;
        await refresh();
