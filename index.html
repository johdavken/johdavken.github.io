...
<head>
  ...
  <style>
    ...
    *{ box-sizing:border-box; }
    ...
    header .sub{ margin-top:6px; color:var(--muted); font-size: var(--font-small); line-height:1.35; }
 ...
 .hopperRow{
  ...
  /* key change: H# column is AUTO, so it hugs the badge */
  grid-template-columns: auto minmax(140px, 1fr) 88px 72px;
  align-items: center;
}

/* small screens: let resin column breathe even more */
@media (max-width: 520px){
  .hopperRow{
    grid-template-columns: auto minmax(120px, 1fr) 78px 66px;
    gap: 6px; /* tighter */
  }

  /* optional: hide the word "Track" on phones to save space */
  .trackLabel{ display:none; }
    ...
    .toggle.on::after{
      /* shift equals (track width - knob - 6px) */
...
<header>
  <h1>Resin Timer</h1>
  <div class="sub">
    Only as accurate as your hopper weights. v01.33
  </div>
</header>
...
<main>
  <section class="card">
    ...
    <div class="grid3">
      <div>
        ...
        <div class="tiny">Line speed * film width * gauge * .048</div>
      </div>
      ...
      <div>
        ...
        <select id="pctMode">
          <option value="100" selected>Inputs are % (sum to 100)</option>
    ...
    <div class="grid3" style="margin-top:10px">
      ...
      <div>
        <label for="densitySel">Density</label>
      ...
      <div>
        <div class="muted" style="margin-top:22px">Layer offsets + layer splits are below in “Layer setup”.</div>
      </div>
    </div>
  ...
  <details class="block card" id="layerSetupBlock" open>
    <summary>
      <div class="sumLeft">
        <div class="chev"></div>
        <div style="min-width:0">
          <div class="layerTitle">2) Layer setup (splits + offsets)</div>
          <div class="layerMeta">Set layer % and downcomer offsets</div>
        </div>
      </div>
    ...
    <div class="blockBody">
      <div class="grid2">
        <div>
          <h2 style="margin:0 0 6px 0; font-size:14px;">Layer split</h2>
        ...
        <div>
          <h2 style="margin:0 0 6px 0; font-size:14px;">Downcomer offset (min)</h2>
          <div class="muted">Adds time for downcomer run-down.</div>
          <div id="offsetInputs" class="gridAuto" style="margin-top:10px"></div>
          <div class="tiny"><strong>(blender weight) / layer output * 60</strong></div>
        </div>
      </div>
    </div>
  </details>

  <details class="block card" id="weightsBlock">
    <summary>
      <div class="sumLeft">
        <div class="chev"></div>
        <div style="min-width:0">
          <div class="layerTitle">3) Hopper weights</div>
          <div class="layerMeta">Amount of stored resin in your receiver hopper. Determines accuracy of your timing. </div>
        </div>
      </div>
      <span class="pill">Collapsed</span>
    </summary>
    <div class="blockBody">
      <div class="muted">Enter current resin weight in each hopper. Results need weight + split + track toggle.</div>
      <div id="weightsArea" class="weightsGrid"></div>
    </div>
  </details>

  <section class="card" id="splitsSection">
    <h2>4) Hopper splits (per layer)</h2>
    <div class="muted">Row format: H# • Resin name • Split • Track. Only tracked hoppers appear in Results.</div>
  ...
  <details class="block card" id="resultsBlock" open>
    <summary>
      <div class="sumLeft">
        <div class="chev"></div>
        <div style="min-width:0">
          <div class="layerTitle">5) Results</div>
  ...
  <section class="card" id="resinCalcSection">
    <h2>6) Resin Calculator</h2>
...
<script>
  const APP_VERSION = "0.08-density-singleline-splits";
  ...
  function ensureLayers(){
    ...
    state.layers = names.map(name => {
      ...
      return {
        name,
        layerPct: 0,
        hoppers: Array.from({length:HOPPERS_PER_LAYER}, ()=>({
  ...
  function snapshotPayload(){
    return {
      ...
      layerSetupOpen: $("layerSetupBlock")?.open ?? true,
      weightsOpen: $("weightsBlock")?.open ?? false,
      resultsOpen: $("resultsBlock")?.open ?? true
    };
  }
  ...
  function renderLayerInputs(){
    ...
    state.layers.forEach((L,i)=>{
      ...
      box.innerHTML = `
        ...
        <input id="${id}" type="text" inputmode="decimal" placeholder="0" value="${L.layerPct}" />
      `;
  ...
  function renderSplitsArea(){
    ...
    state.layers.forEach((L, li)=>{
      ...
      summary.addEventListener("click",(e)=>{ e.preventDefault(); det.open = !det.open; });
      ...
      if (copyBtn){
        copyBtn.addEventListener("click",(e)=>{
          ...
          const ok = confirm(`Copy split + resin name + track toggles from Layer ${from} → Layer ${to}?`);
          if (!ok) return;
          copyLayer(from, to);
          renderSplitsArea();
          validateAndCompute();
  ...
  function renderResultsFlat(flat, changeoverDate){
    ...
    if (flat.length === 0){
      area.innerHTML = `<div class="muted">No tracked hoppers yet. Turn on Track in “Hopper splits”.</div>`;
      return;
    }
  ...
  function rebuildUIFromState(payloadMaybe){
    ...
    if (payloadMaybe && typeof payloadMaybe === "object"){
      if (typeof payloadMaybe.layerSetupOpen === "boolean") $("layerSetupBlock").open = payloadMaybe.layerSetupOpen;
      if (typeof payloadMaybe.weightsOpen === "boolean") $("weightsBlock").open = payloadMaybe.weightsOpen;
      if (typeof payloadMaybe.resultsOpen === "boolean") $("resultsBlock").open = payloadMaybe.resultsOpen;
    }
  ...
  function hookDetailsPersistence(){
    ["layerSetupBlock","weightsBlock","resultsBlock"].forEach(id=>{
  ...
  $("pctMode").addEventListener("change",(e)=>{
    ...
    rebuildUIFromState(); saveSession();
  });
